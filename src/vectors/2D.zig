...
test "sign" {
    ...
    // Act
    const sv0: Vec2 = Math2D.sign(v0);
    const sv1: Vec2 = Math2D.sign(v1);
    const sv2: Vec2 = Math2D.sign(v2);
    const sv3: Vec2 = Math2D.sign(v3);
    const sv4: Vec2 = Math2D.sign(v4);
    const sv5: Vec2 = Math2D.sign(v5);
    const sv6: Vec2 = Math2D.sign(v6);
    const sv7: Vec2 = Math2D.sign(v7);
    const sv8: Vec2 = Math2D.sign(v8);
...
test "map" {
    ...
    // Act
    const m0: Vec2 = Math2D.map(v0, rn2, rp2, rn1, rp1);
    const m1: Vec2 = Math2D.map(v1, rn2, rp2, rn1, rp1);
    const m2: Vec2 = Math2D.map(v2, rn2, rp2, rn1, rp1);
    const m3: Vec2 = Math2D.map(v3, rn2, rp2, rn1, rp1);
    const m4: Vec2 = Math2D.map(v4, rn2, rp2, rn1, rp1);
    const m5: Vec2 = Math2D.map(v5, rn2, rp2, rn1, rp1);
    const m6: Vec2 = Math2D.map(v6, rn2, rp2, rn1, rp1);
    const m7: Vec2 = Math2D.map(v7, rn2, rp2, rn1, rp1);
    const m8: Vec2 = Math2D.map(v8, rn2, rp2, rn1, rp1);
...
test "clamp" {
    ...
    // Act
    const m0: Vec2 = Math2D.clamp(v0, rn1, rp1);
    const m1: Vec2 = Math2D.clamp(v1, rn1, rp1);
    const m2: Vec2 = Math2D.clamp(v2, rn1, rp1);
    const m3: Vec2 = Math2D.clamp(v3, rn1, rp1);
    const m4: Vec2 = Math2D.clamp(v4, rn1, rp1);
    const m5: Vec2 = Math2D.clamp(v5, rn1, rp1);
    const m6: Vec2 = Math2D.clamp(v6, rn1, rp1);
    const m7: Vec2 = Math2D.clamp(v7, rn1, rp1);
    const m8: Vec2 = Math2D.clamp(v8, rn1, rp1);
...
test "magnitude" {
    ...
    const m: f32 = Math2D.magnitude(v);
...
test "normalize" {
    ...
    // Act
    const n0: Vec2 = Math2D.normalize(v0);
    const n1: Vec2 = Math2D.normalize(v1);
    const n2: Vec2 = Math2D.normalize(v2);
    const n3: Vec2 = Math2D.normalize(v3);

    // Assert
    // TODO Find a proper way to round this to a number of decimal places
    try std.testing.expectEqual(1.0, @round(Math2D.magnitude(n0)));
    try std.testing.expectEqual(1.0, @round(Math2D.magnitude(n1)));
    try std.testing.expectEqual(1.0, @round(Math2D.magnitude(n2)));
    try std.testing.expectEqual(1.0, @round(Math2D.magnitude(n3)));
}
test "lerp" {
    @setFloatMode(.optimized);
    @setRuntimeSafety(false);
    ...
    for (0..len) |i| {
        ...
        testVals[i] = Math2D.lerp(v0, v1, t_vec);
...
test "distanceSquared" {
    ...
    // Act
    const d: f32 = Math2D.distanceSquared(v0, v1);
...
test "distance" {
    ...
    // Act
    const d: f32 = Math2D.distance(v0, v1);
...
test "direction" {
    ...
    // Act
    const a01: Vec2 = Math2D.direction(v0, v1);
    const a02: Vec2 = Math2D.direction(v0, v2);
    const a03: Vec2 = Math2D.direction(v0, v3);
    const a04: Vec2 = Math2D.direction(v0, v4);
    // Assert
    try std.testing.expectEqual(comptime @sqrt(Vec2{ 2, 2 }) / Vec2{ 2, 2 }, Math2D.direction(v0, Vec2{ 1, 1 }));
    try std.testing.expectEqual(Vec2{ 1, 1 }, Math2D.sign(a01));
    try std.testing.expectEqual(Vec2{ -1, 1 }, Math2D.sign(a02));
    try std.testing.expectEqual(Vec2{ -1, -1 }, Math2D.sign(a03));
    try std.testing.expectEqual(Vec2{ 1, -1 }, Math2D.sign(a04));
    try std.testing.expectEqual(1.0, Math2D.magnitude(a01));
    try std.testing.expectEqual(1.0, Math2D.magnitude(a02));
    try std.testing.expectEqual(1.0, Math2D.magnitude(a03));
    try std.testing.expectEqual(1.0, Math2D.magnitude(a04));
}
